...
public class CraftingMappings implements ICraftingMappings {
    ...
    protected Set<ICriteria> availableCriteria; //List of all the criteria
        ...
        for (ITrigger trigger : triggers) {
            ...
            if (allFired) {
                ...
                for (ITrigger criteriaTrigger : allTriggers) {
                    forRemovalFromActive.add(criteriaTrigger); //Mark these triggers for removal from the active list
                }
                    ...
                    if (completedCritera.keySet().containsAll(unlocked.getRequirements())) {
                        ...
                        if (!containsAny(unlocked.getConflicts())) {
                            ...
                            for (ITrigger newTrigger : unlocked.getTriggers()) {
                                toAddToActive.add(newTrigger); //Mark these triggers to be added to the active list
                            }
                        }
                    }
                }
                ...
                if (amount < max) { //Removes all the triggers from completed
                    for (ITrigger triggerz : criteria.getTriggers()) {
                        completedTriggers.remove(triggerz);
                        toAddToActive.add(triggerz); //Readd the triggers to active
                    }
                }
            }
        }
    ...
    @Override
    public void remap() {
        availableCriteria = new HashSet(); //Recreate the available mappings
